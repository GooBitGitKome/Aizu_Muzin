...
import AMujinABI from "/Users/Bushi/Documents/GitHub/web3Test/test3/src/contracts/AizuMujin.json";
...
export default function Dapp() {
    return (
        <>
            ...
            <br/><br/><br/><br/>
            <DepositToken _sendTokenToAizuMujin={_sendTokenToAizuMujin} />
...
async function _sendTokenToAizuMujin(){
    let amount = document.getElementById("transactionInpt").value;
    if(!isNaN(amount)){
        try{
             await tokenContract.contract.connect(signer).transfer(AMujinAddress,Number(amount));
        }catch(error){
